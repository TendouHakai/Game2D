...
HRESULT Audio::initialize()
{
    ...
    hFile = CreateFile("sound/metalslug3.xgs", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    if (hFile)
    {
    ...
    hFile = CreateFile("sound/Wave Bank.xwb", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    ...
    result = E_FAIL;    // default to failure code, replaced on success
    hFile = CreateFile("sound/Sound Bank.xsb", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, NULL);
    if (hFile != INVALID_HANDLE_VALUE)
    {
        fileSize = GetFileSize(hFile, NULL);
        if (fileSize != -1)
